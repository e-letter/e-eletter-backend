...
func (h *MasterDataHandler) GetMajors(c *gin.Context) {
	...
	if err != nil {
		response.Error(c, http.StatusInternalServerError, "Gagal mengambil data jurusan")
		return
	}
	response.Success(c, http.StatusOK, "Berhasil mengambil data jurusan", majors)
}

func (h *MasterDataHandler) GetMajor(c *gin.Context) {
	id, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		response.Error(c, http.StatusBadRequest, "ID jurusan tidak valid")
		return
	}
	...
	if major == nil {
		response.Error(c, http.StatusNotFound, "Jurusan tidak ditemukan")
		return
	}
	response.Success(c, http.StatusOK, "Berhasil mengambil data jurusan", major)
}
